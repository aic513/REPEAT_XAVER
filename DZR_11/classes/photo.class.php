...
class photo {
    ...
    public function upload_image($startup, $file,$base) {
        if (!$this->check_upload($file)) { //сюда,если вернет FALSE
            return FALSE;
        } else {
            $type = explode('/', $file['type']);
            $base->send_img($startup, $type[1]);
            $images = $base->get_last_img($startup);
    ...
    public function show_gallery($startup,$base) {
        $images = $base->get_img($startup);
        if (!empty($images)) {
